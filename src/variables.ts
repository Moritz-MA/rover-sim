...
[startpoint_lat, startpoint_lon, target_lat, target_lon, target_b_lat, target_b_lon] = [1.0000, 0.99997, 1.00005, 1.000008, 0.99994, 1.0001]
...
for (let index = 0; index < 3; index++) {
  ...
  if (target_lon > target_b_lon) {
    location_arr.push(
      { 'latitude': target_lat, 'longitude': target_b_lon + (index / 20000), 'label': checkpoint_a },
...
const loop: ControlLoop = ({ location, heading, clock, proximity, targetFinderSignal }, { engines, steering }) => {
  ...
  if (checkpoint == 1) {
    startpoint_lat = location_arr[1].latitude;
    startpoint_lon = location_arr[1].longitude;
    target_lat = location_arr[2].latitude;
    target_lon = location_arr[2].longitude;
  }
